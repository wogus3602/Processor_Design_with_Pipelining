...
void Init();
unsigned int Mux3(char signal, unsigned int a_0, unsigned int b_1, unsigned int c_2);
	...
	*/
	...
	while (pc <= 64)
	{
		...
		ALU_func(ALU_control,
			Mux3(ForwardA, id_ex.Read_data_1, Mux(mem_wb.WB.MemtoReg, mem_wb.alu_result, mem_wb.read_data), ex_mem.alu_result),
			Mux(id_ex.EX.ALUSrc, Mux3(ForwardB, id_ex.Read_data_2, Mux(mem_wb.WB.MemtoReg, mem_wb.alu_result, mem_wb.read_data), ex_mem.alu_result), id_ex.inst_ext_32));

		ex_mem.mux3_result = Mux3(ForwardB, id_ex.Read_data_2, Mux(mem_wb.WB.MemtoReg, mem_wb.alu_result, mem_wb.read_data), ex_mem.alu_result);
...
void ID_EX_Init(unsigned int inst_20_16, unsigned int inst_15_11, unsigned int inst_15_0, unsigned int inst_5_0) {
	...
	id_ex.registerRd = if_id.registerRd;
}
...
void IF_ID_Init(unsigned int inst, unsigned int pc_add_4)
{
	if (!Stall) {
		...
		if_id.registerRs = (inst & 0x03e00000) >> 21; // rs
		if_id.registerRt = (inst & 0x001f0000) >> 16; // rt
		if_id.registerRd = (inst & 0x0000f800) >> 11; // rd		
	}
}
void Init() {
	control.RegDst = 0;
...
void Forwarding_unit() {
	ForwardA = 0;
	ForwardB = 0;
	if (ex_mem.WB.RegWrite && (ex_mem.registerRd != 0) && (ex_mem.registerRd == id_ex.registerRs)) ForwardA = 2;
	if (ex_mem.WB.RegWrite && (ex_mem.registerRd != 0) && (ex_mem.registerRd == id_ex.registerRt)) ForwardB = 2;
	if (mem_wb.WB.RegWrite && (mem_wb.registerRd != 0) && !(ex_mem.WB.RegWrite && (ex_mem.registerRd != 0) && (ex_mem.registerRd != id_ex.registerRs)) && (mem_wb.registerRd == id_ex.registerRs)) ForwardA = 1;
	if (mem_wb.WB.RegWrite && (mem_wb.registerRd != 0) && !(ex_mem.WB.RegWrite && (ex_mem.registerRd != 0) && (ex_mem.registerRd != id_ex.registerRt)) && (mem_wb.registerRd == id_ex.registerRt)) ForwardB = 1;
}

void Hazard_detection_unit() {
	if (id_ex.M.MemRead && ((id_ex.registerRt == if_id.registerRs) || (id_ex.registerRt == if_id.registerRt))) {
		Stall = 1;
	}
}
